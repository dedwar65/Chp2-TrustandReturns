...
forvalues r = 1/`=_N' {
    ...
    local reg_s = string(region_[`r'], "%9.0f")
...
file write fh "\begin{table}[htbp]\centering" _n "\caption{Bin counts by region (2020)}" _n "\label{tab:bin_counts_region_2020}" _n "\begin{tabular}{rlr}\toprule" _n "Region (code) & Region & Obs \\\\ \midrule" _n
forvalues r = 1/`=_N' {
    local r_s = string(region_2020[`r'], "%9.0f")
    local rlab = region_label[`r']
    local n_s = string(n[`r'], "%9.0f")
...
file write fh "\begin{table}[htbp]\centering" _n "\caption{Mean trust by region (2020)}" _n "\label{tab:trust_mean_by_region_2020}" _n "\begin{tabular}{rlrr}\toprule" _n "Region (code) & Region & Mean trust & Obs \\\\ \midrule" _n
forvalues r = 1/`=_N' {
    local r_s = string(region_2020[`r'], "%9.0f")
    local rlab = region_label[`r']
...
local n4 0
forvalues r = 1/`=_N' {
    if region_2020[`r'] == 1 local n1 = n[`r']
    if region_2020[`r'] == 2 local n2 = n[`r']
    if region_2020[`r'] == 3 local n3 = n[`r']
    if region_2020[`r'] == 4 local n4 = n[`r']
}
...
forvalues r = 1/`=_N' {
    local r_s = string(region_[`r'], "%9.0f")
    local rlab = region_label[`r']
    local n_s = string(n[`r'], "%9.0f")
    file write fh "`r_s' & `rlab' & `n_s' \\\\" _n
}
...
* ---------------------------------------------------------------------
* Shares summary (2002 and 2022) — pipeline vars only: share_core, share_ira (m3), share_residential (same as 07)
* ---------------------------------------------------------------------
...
local _wkeep "wealth_core_2002 wealth_ira_2002 wealth_total_2002 wealth_core_2022 wealth_ira_2022 wealth_total_2022"
...
keep hhidpn `_wkeep' ///
    ...
    share_m3_bond_2002 share_m3_stk_2002 share_m3_re_2002 share_m3_bus_2002 share_m3_pri_res_2002 share_m3_sec_res_2002 share_m3_ira_2002 ///
    share_m3_bond_2022 share_m3_stk_2022 share_m3_re_2022 share_m3_bus_2022 share_m3_pri_res_2022 share_m3_sec_res_2022 share_m3_ira_2022
* Fallback: share_core = bond+stk+re+bus; share_residential = pri_res+sec_res (if not in pipeline)
foreach y in 2002 2022 {
    capture confirm variable share_core_`y'
    if _rc {
    ...
    if _rc {
        egen double share_residential_`y' = rowtotal(share_m3_pri_res_`y' share_m3_sec_res_`y'), missing
    }
}
* Keep only vars needed for graphs (wealth_coreira when present)
local _wkeep2 "wealth_core_2002 wealth_ira_2002 wealth_total_2002 wealth_core_2022 wealth_ira_2022 wealth_total_2022"
capture confirm variable wealth_coreira_2002
...
keep hhidpn `_wkeep2' total_income_real_win_2002 total_income_real_win_2022 ///
     share_core_2002 share_residential_2002 share_m3_ira_2002 share_core_2022 share_residential_2022 share_m3_ira_2022
...
foreach y in 2002 2022 {
    * Core only: share_core by wealth percentile
    preserve
    keep hhidpn wealth_core_`y' share_core_`y' `_wght'
    xtile wbin = wealth_core_`y', nq(5)
    collapse (mean) share_core_`y' `wopt', by(wbin)
    graph bar (mean) share_core_`y', over(wbin, relabel(1 "20" 2 "40" 3 "60" 4 "80" 5 "100")) ///
        title("Core share by wealth percentile (`y')") ///
        ytitle("Mean share") b1title("Wealth core (pct.)") ///
        `share_region' ysize(4.2) xsize(6.6)
    graph export "${DESCRIPTIVE}/Figures/share_core_by_wealth_pct_`y'.png", replace
    restore

    * Core + IRA: share_core and share_m3_ira by wealth (core+IRA) percentile; use wealth_coreira_`y' from pipeline when present
    preserve
    local _k "hhidpn wealth_core_`y' wealth_ira_`y' share_core_`y' share_m3_ira_`y' `_wght'"
    capture confirm variable wealth_coreira_`y'
    if !_rc local _k "`_k' wealth_coreira_`y'"
    keep `_k'
    capture confirm variable wealth_coreira_`y'
    if _rc gen double wealth_coreira = wealth_core_`y' + wealth_ira_`y' if !missing(wealth_core_`y') & !missing(wealth_ira_`y')
    if _rc xtile wbin = wealth_coreira, nq(5)
    else xtile wbin = wealth_coreira_`y', nq(5)
    collapse (mean) share_core_`y' share_m3_ira_`y' `wopt', by(wbin)
    graph bar (mean) share_core_`y' share_m3_ira_`y', over(wbin, relabel(1 "20" 2 "40" 3 "60" 4 "80" 5 "100")) stack ///
        title("Core & IRA shares by wealth percentile (`y')") ///
        ytitle("Mean share") b1title("Wealth core+ret. (pct.)") ///
        legend(order(1 "Core" 2 "IRA") cols(2) size(small) region(lstyle(none)) position(6) ring(0)) `share_region' ysize(4.2) xsize(6.6)
    graph export "${DESCRIPTIVE}/Figures/share_core_ira_by_wealth_pct_`y'.png", replace
    restore

    * Total: share_core, share_m3_ira, share_residential by wealth_total percentile
    preserve
    keep hhidpn wealth_total_`y' share_core_`y' share_m3_ira_`y' share_residential_`y' `_wght'
    xtile wbin = wealth_total_`y', nq(5)
    collapse (mean) share_core_`y' share_m3_ira_`y' share_residential_`y' `wopt', by(wbin)
    graph bar (mean) share_core_`y' share_m3_ira_`y' share_residential_`y', over(wbin, relabel(1 "20" 2 "40" 3 "60" 4 "80" 5 "100")) stack ///
        title("Core, IRA & residential by wealth percentile (`y')") ///
        ytitle("Mean share") b1title("Wealth total (pct.)") ///
        legend(order(1 "Core" 2 "IRA" 3 "Residential") cols(3) size(small) region(lstyle(none)) position(6) ring(0)) `share_region' ysize(4.2) xsize(6.6)
    graph export "${DESCRIPTIVE}/Figures/share_core_ira_res_by_wealth_pct_`y'.png", replace
    restore

    * Core only by income percentile
    preserve
    keep hhidpn total_income_real_win_`y' share_core_`y' `_wght'
    xtile ibin = total_income_real_win_`y', nq(5)
    collapse (mean) share_core_`y' `wopt', by(ibin)
    graph bar (mean) share_core_`y', over(ibin, relabel(1 "20" 2 "40" 3 "60" 4 "80" 5 "100")) ///
        title("Core share by income percentile (`y')") ///
        ytitle("Mean share") b1title("Income (pct.)") ///
        `share_region' ysize(4.2) xsize(6.6)
    graph export "${DESCRIPTIVE}/Figures/share_core_by_income_pct_`y'.png", replace
    restore

    * Core + IRA by income percentile
    preserve
    keep hhidpn total_income_real_win_`y' share_core_`y' share_m3_ira_`y' `_wght'
    xtile ibin = total_income_real_win_`y', nq(5)
    collapse (mean) share_core_`y' share_m3_ira_`y' `wopt', by(ibin)
    graph bar (mean) share_core_`y' share_m3_ira_`y', over(ibin, relabel(1 "20" 2 "40" 3 "60" 4 "80" 5 "100")) stack ///
        title("Core & IRA shares by income percentile (`y')") ///
        ytitle("Mean share") b1title("Income (pct.)") ///
        legend(order(1 "Core" 2 "IRA") cols(2) size(small) region(lstyle(none)) position(6) ring(0)) `share_region' ysize(4.2) xsize(6.6)
    graph export "${DESCRIPTIVE}/Figures/share_core_ira_by_income_pct_`y'.png", replace
    restore

    * Total: core, IRA, residential by income percentile
    preserve
    keep hhidpn total_income_real_win_`y' share_core_`y' share_m3_ira_`y' share_residential_`y' `_wght'
    xtile ibin = total_income_real_win_`y', nq(5)
    collapse (mean) share_core_`y' share_m3_ira_`y' share_residential_`y' `wopt', by(ibin)
    graph bar (mean) share_core_`y' share_m3_ira_`y' share_residential_`y', over(ibin, relabel(1 "20" 2 "40" 3 "60" 4 "80" 5 "100")) stack ///
        title("Core, IRA & residential by income percentile (`y')") ///
        ytitle("Mean share") b1title("Income (pct.)") ///
        legend(order(1 "Core" 2 "IRA" 3 "Residential") cols(3) size(small) region(lstyle(none)) position(6) ring(0)) `share_region' ysize(4.2) xsize(6.6)
    graph export "${DESCRIPTIVE}/Figures/share_core_ira_res_by_income_pct_`y'.png", replace
    restore
}
